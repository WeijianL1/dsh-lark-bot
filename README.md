...
智能介入判断和正常回复前从飞书刷新当前群/话题最近最多 50 条公开消息，包含 @ 消息、机器人回复、发言人稳定 ID/显示名及引用关系。历史附件只索引文件名和消息/资源标识，不自动下载；需要时由 `lark_read_chat_history` 继续翻页、`lark_download_attachment` 精确选择并下载。历史只作为上下文，不触发旧任务。新会话补入最近对话；成员身份按 ID 区分，不能把默认用户资料当作全群身份。详见 [群聊上下文](docs/CHAT_CONTEXT.md)。使用现有群消息轮询能力和群历史权限；智能介入启用时优先于旧 `DSH_LARK_GROUP_NO_AT` 开关；没有启用智能介入的群仍要求 @，不会退回逐条回复模式。

等待与超时诊断：聊天历史解析跳过空字段；提问卡投递最多 15 秒，投递成功后等回答最多 2 分钟，过期不会视为批准。详见 [RCA](docs/RCA_WAIT_LATENCY.md)。
